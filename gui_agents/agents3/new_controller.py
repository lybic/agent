...
class NewController:
    ...
    def handle_init_phase(self):
        ...
        try:
            ...
            if pending_subtasks:
                ...
                self.global_state.update_controller_situation(ControllerSituation.GET_ACTION)
            ...
            self.global_state.add_event("controller", "error", f"INIT phase error: {str(e)}")
            self.switch_to_phase(ControllerPhase.PLAN)
    ...
    def handle_et_action_phase(self):
        ...
        try:
            ...
            if subtask_status == SubtaskStatus.FULFILLED.value:
                # 执行成功，进入质检
                self.global_state.update_controller_situation(ControllerSituation.QUALITY_CHECK)
        ...
        try:
            ...
            if latest_gate:
                ...
                if decision == GateDecision.GATE_DONE.value:
                    ...
                    # 检查是否超时
                    if self._is_phase_timeout():
                        logger.warning(f"QUALITY_CHECK phase timeout for subtask {current_subtask_id}")
                        self.global_state.update_controller_situation(ControllerSituation.PLAN)
                        self.switch_to_phase(ControllerPhase.PLAN)
            else:
                # 没有质检记录，继续等待
                ...
                if self._is_phase_timeout():
                    logger.warning(f"QUALITY_CHECK phase timeout (no gate checks) for subtask {current_subtask_id}")
                    self.global_state.update_controller_situation(ControllerSituation.PLAN)
                    self.switch_to_phase(ControllerPhase.PLAN)
                
        except Exception as e:
            logger.error(f"Error in QUALITY_CHECK phase: {e}")
            self.global_state.add_event("controller", "error", f"QUALITY_CHECK phase error: {str(e)}")
            self.switch_to_phase(ControllerPhase.PLAN)
    ...
    def handle_plan_phase(self):
        ...
        try:
            ...
            self.global_state.add_event("controller", "error", f"PLAN phase error: {str(e)}")
            self.switch_to_phase(ControllerPhase.INIT)
    ...
    def handle_supplement_phase(self):
        ...
        try:
            ...
            self.global_state.add_event("controller", "error", f"SUPPLEMENT phase error: {str(e)}")
            self.switch_to_phase(ControllerPhase.GET_ACTION)
    ...
    def _get_situation_for_phase(self, phase: ControllerPhase) -> ControllerSituation:
    ...
    def _should_terminate(self) -> bool:
        ...
        if self.phase_switch_count >= self.max_phase_switches:
            logger.warning(f"Maximum phase switches ({self.max_phase_switches}) reached")
            return True
