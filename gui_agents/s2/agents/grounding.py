...
from gui_agents.s2.core.mllm import LLMAgent
...
class Grounding:
    ...
    def set_grounding_agent(self, agent: ACI):
        """设置grounding agent
        Args:
            agent: ACI, grounding agent
        """
        self.grounding_agent = agent
        
    def assign_coordinates(self, plan: str, obs: Dict, agent: ACI):
        """
        Takes a description based action and assigns the coordinates for any coordinate based action
        Args:
            plan: str, the plan generated by the worker
            obs: Dict, the observation from the environment
            agent: ACI, the agent to assign coordinates to
        """
        try:
            # Extract the function name and args
            action = parse_single_code_from_string(plan.split("Grounded Action")[-1])
            plan_code = sanitize_code(action)
            plan_code = extract_first_agent_function(plan_code)
            exec_code = eval(plan_code)
            return plan_code, exec_code
    ...
    def create_grounding_agent(self, platform: str, engine_params_for_generation: Dict, engine_params_for_grounding: Dict):
        """创建grounding agent
        Args:
            platform: str, (darwin, linux, windows)
            engine_params_for_generation: Dict, 生成模型的参数
            engine_params_for_grounding: Dict, grounding模型的参数
        Returns:
            ACI: grounding agent
        """
...
class OSWorldACI(ACI):
    def __init__(
        ...
        self.grounding_model = LLMAgent(engine_params_for_grounding)
        self.engine_params_for_grounding = engine_params_for_grounding

        # Configure text grounding agent
        self.text_span_agent = LLMAgent(
