...
class NewController:
    ...
    def handle_et_action_phase(self):
        ...
        try:
            ...
            if not subtask:
                logger.warning(
                    f"Subtask {current_subtask_id} not found in ET_ACTION phase"
                )
                self.switch_to_phase(ControllerPhase.INIT)
                return
            ...
            subtask_status = subtask.get("status")
            logger.info(
                f"Subtask {current_subtask_id} status in ET_ACTION: {subtask_status}"
            )
            ...
            if subtask_status == SubtaskStatus.FULFILLED.value:
                ...
                if self._is_phase_timeout():
                    logger.warning(
                        f"ET_ACTION phase timeout for subtask {current_subtask_id}"
                    )
                    self.global_state.update_controller_situation(
                        ControllerSituation.PLAN)
                    self.switch_to_phase(ControllerPhase.PLAN)
