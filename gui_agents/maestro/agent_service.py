...
class AgentServicer(agent_pb2_grpc.AgentServicer):
    ...
    async def GetAgentTaskStream(self, request, context):
        ...
        if not task_info or "queue" not in task_info:
            context.set_code(grpc.StatusCode.NOT_FOUND)
            context.set_details(f"Task with ID {task_id} not found or not ready for streaming.")
            return
    ...
    async def _run_task(self, task_id: str, controller: MainController):
        ...
        try:
            await queue.put(StreamMessage(stage="starting", message="Task starting"))
            
            # Run the blocking function in a separate thread to avoid blocking the event loop
            await asyncio.to_thread(controller.execute_main_loop)
            ...
            if final_state and final_state.status == "fulfilled":
                await queue.put(StreamMessage(stage="finished", message="Task completed successfully"))
            else:
                status = final_state.status if final_state else 'unknown'
                await queue.put(StreamMessage(stage="finished", message=f"Task finished with status: {status}"))
            ...
            if queue:
                await queue.put(StreamMessage(stage="error", message=f"An error occurred: {e}"))
        finally:
            if queue:
    ...
    async def RunAgentInstruction(self, request, context):
        ...
        controller, _ = await self._create_controller_from_request(request, task_id)
        if not controller:
            context.set_code(grpc.StatusCode.INTERNAL)
        ...
        async with self.task_lock:
            ...
            task_future = asyncio.create_task(self._run_task(task_id, controller))
            self.tasks[task_id] = {
    ...
    async def _create_controller_from_request(self, request, task_id):
