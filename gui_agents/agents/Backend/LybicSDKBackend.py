# ---------------------------------------------------------------------------
# 4) Cloud desktop / custom device backend using Official Lybic Python SDK
...
# 导入官方Lybic SDK
try:
    from lybic import LybicClient, Sandbox, ComputerUse, dto  # type: ignore
except ImportError:
    raise ImportError(
        "Lybic Python SDK not found. Please install it with: pip install --upgrade lybic"
    )

# 由于Lybic SDK的类型定义可能不完整，我们在需要的地方使用type: ignore
# mypy: disable-error-code="call-arg,attr-defined,arg-type"

log = logging.getLogger(__name__)


class LybicSDKBackend(Backend):
    """
    基于官方Lybic Python SDK的Backend实现
    支持与LybicBackend相同的Action类型，但使用官方SDK替代HTTP调用
    """
    ...
    def __init__(self, 
                 ...
                 max_retries: int = 2,
                 precreate_sid: Optional[str] = None,
                 **kwargs):
        """
        初始化LybicSDKBackend
        ...
        if self.sandbox_id is None:
            ...
            # 使用getattr以防属性名不同
            self.sandbox_id = getattr(new_sandbox, 'id', None) or getattr(new_sandbox, 'sandbox_id', None)
    ...
    def _execute_with_retry(self, action_dto: dto.ComputerUseActionDto) -> dto.SandboxActionResponseDto:
        ...
        """
        async def _execute():
            return await self.computer_use.execute_computer_use_action(  # type: ignore
    ...
    def _click(self, act: Click) -> dto.SandboxActionResponseDto:
        """执行点击操作"""
        click_action = dto.MouseClickAction(  # type: ignore
            type="mouse:click",
    ...
    def _move(self, act: Move) -> dto.SandboxActionResponseDto:
        """执行鼠标移动操作"""
        move_action = dto.MouseMoveAction(  # type: ignore
            type="mouse:move",
            x=dto.PixelLength(type="px", value=act.x),
            y=dto.PixelLength(type="px", value=act.y),
            holdKey=" ".join(act.holdKey) if act.holdKey else ""
    ...
    def _drag(self, act: Drag) -> dto.SandboxActionResponseDto:
        """执行拖拽操作"""
        drag_action = dto.MouseDragAction(  # type: ignore
            type="mouse:drag",
    ...
    def _type(self, act: TypeText) -> dto.SandboxActionResponseDto:
        """执行文本输入操作"""
        type_action = dto.KeyboardTypeAction(  # type: ignore
            type="keyboard:type",
            content=act.text,
    ...
    def _scroll(self, act: Scroll) -> dto.SandboxActionResponseDto:
        ...
        scroll_action = dto.MouseScrollAction(  # type: ignore
            type="mouse:scroll",
            x=dto.PixelLength(type="px", value=act.x),
            y=dto.PixelLength(type="px", value=act.y),
    ...
    def _screenshot(self) -> Image.Image:
        ...
        async def _get_screenshot():
            return await self.sandbox_manager.get_screenshot(self.sandbox_id)  # type: ignore
        
        try:
            url, image, b64_str = self.loop.run_until_complete(_get_screenshot())
            
            # 返回PIL图像，保持与LybicBackend的兼容性
